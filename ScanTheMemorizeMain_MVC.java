...
class Model{
  ...
  public void lightColorSwitchEvent(Button[] mBtnArys, boolean[] lightControler,Background white ,Background green){
  ...
  // 取得日程表資料
  public boolean[] fetchFreeTime(boolean[][] mBolMtx){
  ...
  public void lookUpSetting_mTxf_action(File fileDb, Alert alertEmpty, Alert alert, TextField mTxf, String nameList){
    try{
        //當 DB為不存在 或是DB不是資料夾時
  ...
  public void createSetting_mBtnCreate_action(Alert alertCrash, Alert alertSuccess, TextField mTxfIndex, TextField mTxfRealId, TextField mTxfFirstMeet, TextArea mTxaHistory, boolean[][] mBolMtx){
    try{
        ...
        boolean[] freeTime = fetchFreeTime(mBolMtx); //TODO
  ...
  public void updateSetting_mBtnUpdate_action(Alert alertCrash, Alert alertConfirm, File fileDb, TextField mTxfIndex, TextField mTxfRealId, TextField mTxfFirstMeet, String nameList){
    try{
        //新檔案的名稱存在這邊
        String oldDataName = "";
...
//妥善查詢功能
  public void lookUpSetting(TextField mTxf){
    ...
    mTxf.setOnAction(e->{
      ml.lookUpSetting_mTxf_action(fileDb, alertEmpty, alert, mTxf, nameList);
    }); //EVENT END
  ...
  public void createSetting(Button mBtnCreate){
    ...
    mBtnCreate.setOnAction(e->{
      ml.createSetting_mBtnCreate_action(alertCrash, alertSuccess, mTxfIndex, mTxfRealId, mTxfFirstMeet, mTxaHistory, mBolMtx);
    });
  }
  //妥善更新功能
  public void updateSetting(Button mBtnUpdate){
    ...
    mBtnUpdate.setOnAction(e->{
      ml.updateSetting_mBtnUpdate_action(alertCrash,alertConfirm,fileDb, mTxfIndex, mTxfRealId, mTxfFirstMeet, nameList);
    });
  }
}
...
// Controller
public class ScanTheMemorizeMain_MVC extends Application{
  Model ml = new Model();
  View vi = new View();
  public void start(Stage primaryStage){
    ...
    //設定時間表按鈕事件
    ml.lightColorSwitchEvent(vi.mBtnMtx[1], vi.mBolMtx[1], vi.white, vi.green);
    ml.lightColorSwitchEvent(vi.mBtnMtx[2], vi.mBolMtx[2], vi.white, vi.green);
    ml.lightColorSwitchEvent(vi.mBtnMtx[3], vi.mBolMtx[3], vi.white, vi.green);

    //妥善查詢功能
    vi.lookUpSetting(vi.mTxfLookUp);

    //妥善新建功能
    vi.createSetting(vi.mBtnCreate);

    //妥善更新功能  //TODO 未處理
    vi.updateSetting(vi.mBtnUpdate);
